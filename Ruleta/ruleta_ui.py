    ...
    def paintEvent(self, a0):
        ...
        for i, number in enumerate(wheel_numbers):
            ...
            painter.drawPie(QRectF(-radius, -radius, radius * 2, radius * 2),
                          int(start_angle * 16), int(angle_per_number * 16))
...
class RouletteWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.history_label.setStyleSheet("background-color: rgba(0, 0, 0, 0.5); padding: 10px; border-radius: 5px;")
    ...
    def place_number_bet(self, number: int):
        ...
        if bet and self.game.place_bet(bet):
            self.update_display()
            self.result_label.setText(f"Apuesta ${self.current_chip_value} al número {number}")
        else:
    ...
    def place_dozen_bet(self, dozen: int):
        ...
        if bet and self.game.place_bet(bet):
            ...
            self.result_label.setText(f"Apuesta ${self.current_chip_value} a docena {ranges[dozen]}")
        else:
            QMessageBox.warning(self, "Error", "No puedes colocar esta apuesta.")
    ...
    def spin_wheel(self):
        ...
        QTimer.singleShot(3000, lambda: self.show_result(winning_number, total_winnings))
    ...
    def show_result(self, winning_number: int, total_winnings: int):
        ...
        if total_winnings > 0:
            result_text = f"¡Ganó el {winning_number} {color_text}!\n¡Ganaste ${total_winnings}!"
        else:
            result_text = f"Ganó el {winning_number} {color_text}.\nPerdiste esta ronda."
