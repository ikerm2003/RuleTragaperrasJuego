...
class PokerWindow(QMainWindow):
    ...
    def __init__(
        ...
        self.table.register_ui_callback(
            "highlight_player",
            lambda player_position: self.highlight_current_player(player_position),
        )
        self.table.register_ui_callback(
            "show_actions",
            lambda player_position, actions: self.show_action_buttons(
                player_position, actions
            ),
        )
    ...
    def show_config_dialog(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Info", "Configuración no disponible en este momento."
            )
    ...
    def create_community_cards_section(self):
        ...
        community_frame.setFixedSize(
            self.get_scaled_size(400), self.get_scaled_size(140)
        )
        ...
        for i, (row, col, position) in enumerate(seat_layout):
            if i < len(self.table.players):
                ...
                self.table_layout.addWidget(
                    player_frame, row, col, Qt.AlignmentFlag.AlignCenter
                )
    ...
    def create_action_panel(self, main_layout: QVBoxLayout):
        ...
        self.fold_button = self.create_action_button(
            get_text("fold"), action=PlayerAction.FOLD
        )
        self.check_call_button = self.create_action_button(
            get_text("check"), click_handler=self.on_check_call_clicked
        )
        self.raise_button = self.create_action_button(
            get_text("raise"), action=PlayerAction.RAISE
        )
    ...
    def execute_player_action(self, action: PlayerAction):
        ...
        if not self.table.players or self.table.current_player >= len(
            self.table.players
        ):
            return
    ...
    def show_available_actions(
        ...
        min_raise = (
            self.table.current_bet + self.table.min_raise
            if hasattr(self.table, "min_raise")
            else 0
        )
        max_raise = (
            current_player.chips + current_player.current_bet
            if hasattr(current_player, "chips")
            and hasattr(current_player, "current_bet")
            else 0
        )
        if can_raise:
    ...
    def animate_bet_change(self, bet_label: QLabel):
        ...
        duration = (
            int(300 / self.config.get_animation_speed())
            if self.config.get_animation_speed() > 0
            else 300
        )
    ...
    def animate_player_highlight(self, player_position: int):
        ...
        if player_position < len(self.player_displays):
            ...
            duration = (
                int(200 / self.config.get_animation_speed())
                if self.config.get_animation_speed() > 0
                else 200
            )
            animation.setDuration(duration)
    ...
    def animate_card_deal(self, card_label: QLabel, delay: int = 0):
        ...
        duration = (
            int(400 / self.config.get_animation_speed())
            if self.config.get_animation_speed() > 0
            else 400
        )
        animation.setDuration(duration)
    ...
    def animate_pot_update(self):
        ...
        duration = (
            int(250 / self.config.get_animation_speed())
            if self.config.get_animation_speed() > 0
            else 250
        )
            ...
            if i < len(self.table.players):
                ...
                if current_bet > 0:
                    ...
                    if hasattr(frame, "bet_label"):
                        if (
                            hasattr(frame.bet_label, "_last_bet")
                            and frame.bet_label._last_bet != current_bet
                        ):
                ...
                reveal_cards = len(player.hand) >= 2 and (
                    player.is_human or self.reveal_all_hands
                )
    ...
    def load_card_image(self, card: PokerCard) -> QPixmap:
        ...
        painter.drawText(
            card_width // 2 - value_width // 2, card_height // 2, card.value
        )
        ...
        painter.drawText(
            card_width // 2 - symbol_width // 2, int(card_height * 0.75), symbol
        )
